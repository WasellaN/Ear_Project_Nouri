...
#letter to LETTER function
upperchange <- function(lower_vec){
  LETTERS[match(lower_vec, letters)]
}
...
sowing_climate <- climate %>% group_by(DFG_year, sowing_date) %>%
  slice(1) %>%
...
original_climate <- climate %>%
  group by
  select(DayTime, DFG_year, DailyMean_Temperature, sowing_dates) %>%
  distinct()

thermal_climate <- left_join(original_climate, sowing_climate)
